...
static inline void _rb_init(struct rbt_tree *rbt) { rbt->rbt_root = NULL; }
...
#define RB_PROTOTYPE(_name, _type, _field, _cmp)                               \
  ...
  __attribute__((__unused__)) static inline void _name##_RB_INIT(              \
    ...
    _rb_init(&head->rbh_root);                                                 \
                                                                               ...
                                                                               \
  __attribute__((__unused__)) static inline struct _type *_name##_RB_INSERT(   \
      struct _name *head, struct _type *elm) {                                 \
    return (struct _type *)_rb_insert(_name##_RB_TYPE, &head->rbh_root, elm);  \
  }                                                                            \
                                                                               \
  __attribute__((__unused__)) static inline struct _type *_name##_RB_REMOVE(   \
      struct _name *head, struct _type *elm) {                                 \
    return (struct _type *)_rb_remove(_name##_RB_TYPE, &head->rbh_root, elm);  \
  }                                                                            \
                                                                               \
  ...
  __attribute__((__unused__)) static inline struct _type *_name##_RB_NFIND(    \
      const struct _name *head, const struct _type *key) {                     \
    return (struct _type *)_rb_nfind(_name##_RB_TYPE, &head->rbh_root, key);   \
  }                                                                            \
                                                                               \
...
#define RB_FOREACH(_e, _name, _head)                                           \
  for ((_e) = RB_MIN(_name, (_head)); (_e) != NULL; (_e) = RB_NEXT(_name, (_e)))
...
#define RB_FOREACH_REVERSE(_e, _name, _head)                                   \
  for ((_e) = RB_MAX(_name, (_head)); (_e) != NULL; (_e) = RB_PREV(_name, (_e)))
