...
static int dev_range_cmp(const struct dev_range_entry *a,
                         const struct dev_range_entry *b) {
...
void usage_exit() {
  error_exit(
      "Usage: btstats [-h] [-f <file>] [-r <reader>] [-t] [-d <file>] [-i "
      "<file>] [<trace> .. <trace>]\n\n"
      "Options:\n"
...
void analyze_device(char *dev, struct time_range_list *ranges,
  ...
  SLIST_FOREACH(r, ranges, entries) { range_finish(r, ps, r->ps, dev); }
}
