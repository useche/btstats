...
void reqsize_print_results(const void *data) {
  ...
  if (rsd->reqs)
    printf("Reqs. #: %lld Reads: %lld (%.1f%%) Size:(min: %lld avg: %f max: "
           "%lld (blks))\n",
           (long long)rsd->reqs, (long long)rsd->reads,
           100 * ((double)rsd->reads) / rsd->reqs, (long long)rsd->min,
           ((double)rsd->total_size) / rsd->reqs, (long long)rsd->max);
}
...
void reqsize_destroy(struct plugin *p) { free(p->data); }
