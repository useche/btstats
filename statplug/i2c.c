...
static void oio_change(struct i2c_data *i2c, const struct blk_io_trace *t,
                       int inc) {
  /* allocate oio space if the one I had is over */
  if (i2c->outstanding + 1 >= i2c->oio_size) {
    i2c->oio = realloc(i2c->oio,
                       (i2c->oio_size + OIO_ALLOC) * sizeof(struct oio_data));
  ...
  RB_FOREACH(se, sector_tree_head, i2c->is) { add_to_matrix(se, i2c); }
...
void i2c_print_results(const void *data) {
  ...
  if (i2c->oio_hist_f) {
    for (i = 1; i <= i2c->maxouts; i++) {
      ...
      gsl_histogram_fprintf(i2c->oio_hist_f, i2c->oio[i].op[READ], "%g", "%g");
      fprintf(i2c->oio_hist_f, "\nwrite: %d\n", i);
      gsl_histogram_fprintf(i2c->oio_hist_f, i2c->oio[i].op[WRITE], "%g", "%g");
    }
  }
...
void i2c_init(struct plugin *p, struct plugin_set *__un1,
              struct plug_args *pa) {
