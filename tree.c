...
static inline void rbe_if_augment(const struct rb_type *t,
                                  struct rb_entry *rbe) {
  if (t->t_augment != NULL)
    rbe_augment(t, rbe);
}
...
static inline void rbe_remove_color(const struct rb_type *t,
  ...
  while ((rbe == NULL || RBE_COLOR(rbe) == RB_BLACK) && rbe != RBH_ROOT(rbt) &&
         parent) {
    if (RBE_LEFT(parent) == rbe) {
      ...
      if ((RBE_LEFT(tmp) == NULL || RBE_COLOR(RBE_LEFT(tmp)) == RB_BLACK) &&
          (RBE_RIGHT(tmp) == NULL || RBE_COLOR(RBE_RIGHT(tmp)) == RB_BLACK)) {
...
static inline struct rb_entry *rbe_remove(const struct rb_type *t,
                                          struct rbt_tree *rbt,
                                          struct rb_entry *rbe) {
